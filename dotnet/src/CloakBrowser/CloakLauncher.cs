...
public static class CloakLauncher
{
    ...
    public static async Task<CloakBrowserHandle> LaunchAsync(LaunchOptions? options = null)
    {
        ...
        string binaryPath = await Download.EnsureBinaryAsync().ConfigureAwait(false);
        ...
        try
        {
            ...
            var humanCfg = options.Humanize
                ...
                : null;
            return new CloakBrowserHandle(playwright, browser, options.Humanize, humanCfg);
        }
        catch
        {
            playwright.Dispose();
    ...
    private static BrowserNewContextOptions BuildContextOptions(LaunchContextOptions options)
    {
        ...
        if (options.NoViewport)
            ...
            ctx.ViewportSize = new ViewportSize { Width = options.Viewport.Value.Width, Height = options.Viewport.Value.Height };
        else
            ctx.ViewportSize = new ViewportSize { Width = Config.DefaultViewportWidth, Height = Config.DefaultViewportHeight };
